/**
 * @file benlib_intro.h
 * @author Ramon Santamaria (@raysan5), update by Bensuperpc (bensuperpc@gmail.com)
...
void play_intro_benlib(const int screenWidth, const int screenHeight)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            if (state == 0)
            {
                if ((framesCounter/15)%2) DrawRectangle(logoPositionX, logoPositionY, 16, 16, DARKPURPLE);
            }
            else if (state == 1)
            {
